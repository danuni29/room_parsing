...
colums = ["item_id", "sales_type", "deposit", "rent", "address1", "random_location", "manage_cost", 'images_thumbnail']
df = pd.DataFrame(items)[colums]
df = df[df["address1"].str.contains(addr)].reset_index(drop=True)
df = df.rename(columns={"address1": "주소", "sales_type": "유형", "deposit": "보증금", "rent": "월세", "manage_cost": "관리비", 'images_thumbnail': "사진"})
...
output_filename = os.path.join(output_dir, 'zigbang_parsing_data.xlsx')
